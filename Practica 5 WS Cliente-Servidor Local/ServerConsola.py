...
class MyHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        if self.path.startswith('/listas/'):#peticion de Spotify
            pais = self.path[8:]
            if pais in listas:
        ...
        elif self.path.startswith('/temperature/'):#peticion de temperatura
            pais = self.path[13:]
            if pais in temperaturas:
                ...
                self.end_headers()
                self.wfile.write("País no encontrado.".encode())  # Codificar la cadena a bytes
        elif self.path.startswith('/informacion/'):#peticion de Geonames
            pais = self.path[13:]
